...
class UDPChatServer:
    def __init__(self, server_address="0.0.0.0", server_port=9001, session_lifetime=30, check_interval=10, alert_message=20):
        ###########
        # 設定情報
        ###########
        ...
        if not username in self.users_dict:
            ...
            if message != "exit":
                self.users_dict[username][
                    1
                ] = self.session_lifetime_seconds  # メッセージを受信するたびに生存時間を戻す
    ...
    def check_users_lifetime(self):
        ...
        while True:
            ...
            for username in list(
                ...
                elif self.lifetime_check_interval_seconds < remaining_lifetime <= self.lifetime_alert_seconds :
    ...
    def broadcast(self, data):
        """全クライアントにメッセージを送信"""
        for user, (addr, _) in self.users_dict.items():
    ...
    def notify_exit(self, username):
        ...
        adminname_len = len(adminname_bytes)
        send_data = (
            bytes([adminname_len]) + adminname_bytes + goodbye_message_bytes
        )
